...
public:
     ...
     * @param[in] address 地址
     */
    static Socket::ptr CreateTcp(HPGS::Address::ptr address);

    /**
     * @brief 创建UDP Socket(满足地址类型)
     * @param[in] address 地址
     */
    static Socket::ptr CreateUdp(HPGS::Address::ptr address);

    /**
     * @brief 创建IPv4 的 TCP Socket
     */
    ...
    /**
     * @brief 创建IPv6 的 UDP Socket
     */
    static Socket::ptr CreateUdpSocket6();

    /**
     * @brief 创建Unix 的 TCP Socket
     */
    static Socket::ptr CreateUnixTcpSocket();

    /**
     * @brief 创建Unix 的 UDP Socket
     */
    ...
    /**
     * @brief 设置socketopt @see setsockopt
     */
    ...
    /**
     * @brief 发送数据
     * @param[in] buffer 待发送数据的内存(iovec数组)
     * @param[in] length 待发送数据长度(iovec长度)
     * @param[in] flags 标志字
     * @return 
     *      @retval > 0 发送成功对应大小的数据
     *      @retval = 0 socket被关闭
...
protected:
    ...
    bool m_isConnected;
    /// 本地地址
    HPGS::Address::ptr m_localAddress;
    /// 远端地址
    HPGS::Address::ptr m_remoteAddress;
...
public:
    ...
    static SSLSocket::ptr CreateTcp(HPGS::Address::ptr address);
