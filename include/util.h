#ifndef __HPGS_UTIL_H__
#define __HPGS_UTIL_H__
...
std::string Time2Str(time_t = time(0), const std::string& format = "%Y-%m-%d %H:%M:%S");
...
public:
    ...
    static bool Rm(const std::string& dirname);
...
public:
    ...
    static std::string UrlEncode(const std::string& std, bool space_as_plus = true);
    static std::string UrlDecode(const std::string& std, bool space_as_plus = true);
...
/**
 * @brief 在 arr的[begin, end)之间 查找v
 *        if rt > 0, 存在，返回对应index
 *        if rt < 0, 不存在,返回应该存在时index
 */
template<class T>
...
template<class T>
bool ReadFromStream(std::istream& is, T& t){
...
bool ReadFromStreamWithSpeed(std::istream& is, const T& v
                            , const uint64_t& speed = -1){
    if(is){
        retuirn ReadFixFromStreamWithSpeed(is, (char*)&v[0], sizeof(T) * v.size(), speed);
    }
    return false;
}
