...
bool Socket::reconnect(uint64_t timeout_ms){
    if(!m_remoteAddress){
        HPGS_LOG_ERROR(g_logger) << "reconnect m_remoteAddress is null";
        return false;
    }
...
bool Socket::connect(const HPGS::Address::ptr addr, uint64_t timeout_ms){
    ...
    if(HPGS_UNLIKELY(addr->getFamily() != m_family)){
        HPGS_LOG_ERROR(g_logger) << "connect sock.family(" << m_family
                                 << ") addr.family(" << addr->getFamily()
                                 << ") not equal, addr = " << addr->toString();
        return false;
    }
    ...
    if(timeout_ms == (uint64_t)-1){
        if(::connect(m_sock, addr->getAddr(), addr->getAddrlen())){
            HPGS_LOG_ERROR(g_logger) << "sock = " << m_sock << " connect(" << addr->toString()
...
bool Socket::listen(int backlog) {
    if(!isValid()) {
        HPGS_LOG_ERROR(g_logger) << "listen error sock=-1";
        return false;
    }
    if(::listen(m_sock, backlog)) {
        HPGS_LOG_ERROR(g_logger) << "listen error errno=" << errno
            << " errstr=" << strerror(errno);
        return false;
    }
    return true;
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if(getpeername(m_sock, result->getAddr(), &addrlen)) {
        //HPGS_LOG_ERROR(g_logger) << "getpeername error sock=" << m_sock
...
Address::ptr Socket::getLocalAddress() {
    ...
    if(getsockname(m_sock, result->getAddr(), &addrlen)) {
        HPGS_LOG_ERROR(g_logger) << "getsockname error sock=" << m_sock
            << " errno=" << errno << " errstr=" << strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
    if(m_family == AF_UNIX) {
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, HPGS::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, HPGS::IOManager::WRITE);
}
...
void Socket::newSock() {
    ...
    if(HPGS_LIKELY(m_sock != -1)) {
        initSock();
    } else {
        HPGS_LOG_ERROR(g_logger) << "socket(" << m_family
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
    ...
    if(SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
        HPGS_LOG_ERROR(g_logger) << "SSL_CTX_use_certificate_chain_file("
            << cert_file << ") error";
        return false;
    }
    if(SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
        HPGS_LOG_ERROR(g_logger) << "SSL_CTX_use_PrivateKey_file("
            << key_file << ") error";
        return false;
    }
    if(SSL_CTX_check_private_key(m_ctx.get()) != 1) {
        HPGS_LOG_ERROR(g_logger) << "SSL_CTX_check_private_key cert_file="
...
SSLSocket::ptr SSLSocket::CreateTcp(HPGS::Address::ptr address) {
