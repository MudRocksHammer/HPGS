    ...
    for(int i = 0; i < argc; i++){
        if(argv[i][0] == '-'){
            if(strlen(argv[i]) > 1){
                ...
                HPGS_LOG_ERROR(g_logger) << " invalid arg idx = " << i
                << " val = " << argv[i];
                return false;
            }
        }
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const{
    ...
    static HPGS::ConfigVar<std::string>::ptr g_server_work_path = HPGS::Config::Lookup<std::string>("server.work_path");
